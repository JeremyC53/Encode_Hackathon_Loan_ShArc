...
import { CREDIT_LOAN_ABI, ERC20_ABI, CREDIT_LOAN_ADDRESS, USDC_ADDRESS } from "./CreditLoanABI";
...
export const repayLoan = async (loanId: number, amount: number): Promise<string> => {
  try {
    ...
    const currentAllowance = await usdcContract.allowance(signerAddress, CREDIT_LOAN_ADDRESS);
    console.log(currentAllowance,'cuu')
    // Step 2: Approve USDC if needed
    if (currentAllowance < amountInWei) {
      console.log("Approving USDC...");
      const approveTx = await usdcContract.approve(CREDIT_LOAN_ADDRESS, amountInWei);
