...
import { checkAndSetCreditScore } from "../contracts/contractUtils";
...
const EarningsScreen: React.FC = () => {
  ...
  // Check and set credit score on blockchain if wallet is connected
  useEffect(() => {
    const handleCreditScore = async () => {
      // Only proceed if we have a wallet address and a valid credit score
      if (!walletAddress || !creditScore || creditScore === 575) {
        return;
      }

      try {
        console.log(`Checking credit score for ${walletAddress}...`);
        const result = await checkAndSetCreditScore(walletAddress, creditScore);

        if (result.updated) {
          if (result.hasScore) {
            console.log(
              `Credit score updated from ${result.existingScore} to ${creditScore}:`,
              result.transaction
            );
          } else {
            console.log(`Credit score set for the first time:`, result.transaction);
          }
        } else {
          console.log(`Credit score is up to date: ${result.existingScore}`);
        }
      } catch (error) {
        console.error("Failed to check/set credit score:", error);
      }
    };
